...
Locator: getByTestId('todo-titleo')
- Expected  - 3
+ Received  + 1
...
Call log:
  ...
  - waiting for getByTestId('todo-titleo')
    9 × locator resolved to 0 elements

    at /Users/rostandkinfack/playwright/poc-playwright/tests/dashboard.spec.ts:24:51
```
...
```ts
   ...
   18 |
   19 |     // Create 1st todo.
   20 |     await newTodo.fill(TODO_ITEMS[2]);
   21 |     await newTodo.press('Enter');
   22 |
   23 |     // Make sure the list only has one todo item.
>  24 |     await expect(page.getByTestId('todo-titleo')).toHaveText([
      |                                                   ^ Error: Timed out 5000ms waiting for expect(locator).toHaveText(expected)
   25 |       TODO_ITEMS[0]
   26 |     ]);
   27 |
